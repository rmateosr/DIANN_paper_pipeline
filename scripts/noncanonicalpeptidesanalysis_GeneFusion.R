...
# ABOUTME: Analyses non-canonical gene fusion peptides; outputs normalised tables and PDF.
...
# Gene fusion entries do NOT contain ":" in the header (hotspot entries do)
...
gene_fusion_library = data.frame(fread("gene_fusion_peptide_library.tsv", header = F, sep = "\t"))
...
# Keep only sequences confirmed in the fusion library
...
write.table(normalizednumericoutputDIANN_selection, "Peptidomics_Results/fusion_peptides.tsv", sep = "\t", quote = F, col.names = TRUE, row.names = FALSE )
...
myColors <- c(
  ...
  "#66C2A5", "#0033A0", "#F4A6D7", "#FC8D62"
)


# Label: take the last ";" element (most fusion-specific protein group)
...
# Deduplicate: keep dominant charge state per peptide
...
pdf("Peptidomics_Results/fusion_by_gene.pdf", width = 10, height = 15)
...
# Labels can follow two formats: with ":" (older, with fusion coordinates) or without
...
for(cont in 1:length(noncanonLabel)){
  ...
  # dummy row keeps shape legend stable
